...
- **Locked, non-negotiable: strict first-person POV — the character's own face must never be visible in frame.** Confirmed as a real failure mode 2026-08-05: Seedance 2.0 rendered a "waking up" shot as a third-person view of the character's face rather than his own eyes looking out, despite the scene description saying "POV." The word "POV" alone is not sufficient instruction — every video prompt must explicitly state that the camera IS the character's own eyes (body-worn-camera framing) and that their face is never visible. This is now auto-enforced in code via `POV_LOCK_CLAUSE` in `shot_list_builder.build_video_prompt()` — every prompt built through that function gets it automatically, so there is no need (and no reason) to hand-write this instruction per-shot.
