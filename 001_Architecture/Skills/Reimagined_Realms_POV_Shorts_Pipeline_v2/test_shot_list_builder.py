import pytest
from shot_list_builder import build_video_prompt, write_shot_list, POV_LOCK_CLAUSE, build_multi_character_reference_note
...
def test_build_multi_character_reference_note_returns_empty_for_none_or_empty():
    assert build_multi_character_reference_note(None) == ""
    assert build_multi_character_reference_note([]) == ""


def test_build_multi_character_reference_note_tags_images_in_order():
    note = build_multi_character_reference_note(["the main stoker", "a background worker"])

    assert "@Image1 shows the main stoker." in note
    assert "@Image2 shows a background worker." in note
    assert note.index("@Image1") < note.index("@Image2")


def test_build_video_prompt_includes_multi_character_note_when_provided():
    result = build_video_prompt(
        scene_description="Three stokers shovel coal together",
        sound_events="shovels scraping, furnace roar",
        camera_fixed=True,
        character_roles=["the main stoker", "a background worker"],
    )
    assert "@Image1 shows the main stoker." in result
    assert "@Image2 shows a background worker." in result


def test_build_video_prompt_omits_reference_note_when_no_character_roles():
    result = build_video_prompt(
        scene_description="POV walking down a corridor",
        sound_events="footsteps",
        camera_fixed=False,
    )
    assert "@Image" not in result


def test_build_video_prompt_prepends_pov_lock_clause():
    result = build_video_prompt(
        scene_description="POV lying in a narrow crew bunk, waking with a groggy sigh",
        sound_events="groggy sigh, creaking wood bunk",
        camera_fixed=True,
    )
