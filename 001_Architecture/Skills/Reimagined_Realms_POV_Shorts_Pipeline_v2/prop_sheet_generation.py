...
#
# Built 2026-08-08 after a confirmed failure on the Roman Gladiator production:
# a Prop Sheet showing only a shield's decorative front face caused every scene
# needing that shield in-hand to render it backwards, because a held shield's
# front always faces AWAY from the person carrying it — true POV only ever sees
# the back/strap side with the forearm threaded through it. The model had no
# other reference to work from.
#
# Fix: every prop that could appear in a POV shot needs every orientation that
# could actually appear on screen, explicitly labeled — front, back (for any
# prop with an asymmetric front/back or a specific carry orientation), and a
# "held from POV" panel showing the actual grip/carry position with a hand/arm
# in frame (the one deliberate exception to the "no hands" rule below).
#
# Revised 2026-08-08 (round 2): a single generic "held" panel produced a wrong
# thumb orientation on the Scutum Shield's held panel — the shield is always
# carried in the left hand, but the panel gave no hand explicitly, so the model
# had a 50/50 chance of rendering right-hand grip anatomy on a left-hand grip.
# Fixed by splitting "held" into "held_left"/"held_right" so a prop's actual
# carry hand is always explicit, never guessed. Also added an explicit margin
# instruction per Tony's correction that panel labels must never overlap the
# image content itself — every label needs its own reserved blank space.
import argparse
import json
...
def build_prop_sheet_prompt(props: list[dict]) -> str:
    ...
    in a POV shot — most props are carried in exactly one specific hand by convention (e.g. a
    ...
    production's Main Character Sheet, since that's the actual POV carry reference.
    """
    panel_lines = []
    for prop in props:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate a prop consistency reference sheet — front/back/held-from-POV panels per "
                     "prop, for any recurring handheld/worn item the main POV character carries. Never "
                     "generate only a single 'hero' angle for an asymmetric prop (e.g. a shield) — that "
    ...
    parser.add_argument(
        ...
        help="Optional reference image URLs (e.g. the Main Character Sheet, for consistent hand/arm in held panels)",
    )
