...
**Verify the matte, don't just check the file mode.** `Image.open(path).mode == "RGBA"` confirms alpha exists, not that it's clean — AI matting can leave semi-transparent fringing/haloing on fine details. Spot-check edges visually before considering an asset locked. **Composite the result over an actual checkerboard/alpha-test pattern, not just eyeballing the RGBA file in an image viewer** — color contamination inside a translucent subject can be invisible against a plain preview background but obvious once composited over something with contrast.

**Chroma-green causes color-spill contamination on translucent/glowing subjects — flip to near-black for these (confirmed 2026-08-21).** Method 2 (green chroma-screen) works cleanly for opaque/solid subjects (confirmed on Scene 05's filter disc and eye cross-section — crisp edges, zero contamination), but on **semi-transparent volumetric elements** (glowing wave ribbons, light trails, glass/energy effects) the green background light physically "shines through" the translucent pixels in the base render itself, not just at the silhouette edge — Recraft's matting removes the solid green backdrop but cannot undo a hue shift baked into the translucent color values themselves. Caught on Scene 05 (polarization light-wave diagram): an amber-gold wave asset and a teal/amber twisting-wave asset both came back visibly yellow-green after chroma-green + Recraft, confirmed by compositing over a checkerboard. **Fix: regenerate translucent/glowing components on a near-black background instead of chroma-green, then matte as usual** — a dark neutral backdrop doesn't tint transparency the way a saturated color does. Re-run confirmed clean (pure amber-gold, pure teal-cyan, zero contamination). **Practical rule: pick the matting background per-component based on whether that specific element is opaque or translucent/glowing, not one background choice for the whole batch** — Scene 05 used chroma-green for its two opaque components and near-black for its two translucent ones, in the same style-locked set.
