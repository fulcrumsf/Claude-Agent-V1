---
tags: [chatgpt-profile, tony-patterns, content-strategy-youtube]
source: chatgpt-export-2026
conversations: 240
date-synthesized: 2026-05-06
---

# Tony's Content Strategy / YouTube

## How Tony Thinks About This Domain
- Tony treats this as packaging and performance engineering: hook, title, thumbnail, and audience fit matter more than generic advice.
- He usually wants outputs that can be tested immediately, then refined against engagement or clarity.

## Recurring Frameworks & Vocabulary
- Common terms: `youtube`, `tiktok`, `channel`, `viral`, `seo`, `description`, `but`, `add`, `should`, `tags`
- `Bio Ideas fo Kingdoms and Conquerors` — Make the Tiktok Bio 80 characters or less give me 3 hashtags to always use for the best SEO and discoverability I have a youtube channel called "Kingdoms and Conquerors" The cha...
- `Viral Username Generation Help` — Your Niche: Historical Events Reimagined Target Audience 1: Traditionalists Target Audience 2: Modern Innovators  Target Audience 1 Stance on Your Niche:  Positive Views: Tradit...
- `YouTube SEO: Titles, Description,Tags` — # Prompt by AIPRM, Corp. - https://www.aiprm.com/prompts/seo/keywords/1793421520232775680/ Lines starting with # are only comments for humans You must add at the top of the resp...

## Prompt Style Signals
- Requests usually arrive with enough detail to act on directly.
- Tony frequently trims, simplifies, or removes unwanted elements.
- He often wants several variants instead of one final answer.
- He likes structured breakdowns and repeatable frameworks.
- He uses language as a performance lever: packaging, hooks, and conversion matter.

## Key Preferences & Opinions
- Prefers concise, high-signal creative options over theory-heavy strategy.
- Looks for repeatable frameworks he can reuse across channels and posts.

## Related Images
- None linked from image_map.json
