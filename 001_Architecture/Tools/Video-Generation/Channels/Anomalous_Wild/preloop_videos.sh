...
# Output: Video_Clips/scene_XX_looped.mp4 — ready for Remotion single-video playback.
# Uses -stream_loop + re-encode so keyframes are clean and seeking works perfectly.

set -e
BASE="002_Channels/001_Anomalous-Wild/Productions/0001_Bioluminescence_Weapon"
...
for scene in "${!DURATIONS[@]}"; do
  dur="${DURATIONS[$scene]}"
  input="$BASE/Video_Clips/$scene.mp4"
  output="$BASE/Video_Clips/${scene}_looped.mp4"
