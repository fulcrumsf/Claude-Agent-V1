...
def check():
    ...
    for sid in ORIGINAL_CLIPS:
        looped  = BASE / "Video_Clips" / f"{sid}_looped.mp4"
        raw     = BASE / "Video_Clips" / f"{sid}.mp4"
        if looped.exists():
    ...
    for pri in ("critical", "high", "medium"):
        ...
        for e in group:
            ...
            raw    = folder / f"{e['scene_id']}.mp4"
            looped = folder / f"{e['scene_id']}_looped.mp4"
            if looped.exists():
                done_l += 1
            elif raw.exists():
                done_v += 1
    ...
    # ── Images ────────────────────────────────────────
    imgs_ready = sum(1 for e in img_entries if (Path(e["output_folder"]) / f"{e['scene_id']}.png").exists())
    print(f"\n── FLUX PRO IMAGES ({imgs_ready}/{len(img_entries)} done) ──")
    for e in img_entries:
        img_path = Path(e["output_folder"]) / f"{e['scene_id']}.png"
        if img_path.exists():
    ...
    # ── Summary ───────────────────────────────────────
    orig_ready  = sum(1 for s in ORIGINAL_CLIPS if (BASE/"Video_Clips"/f"{s}_looped.mp4").exists())
    new_looped  = sum(1 for e in vid_entries if (Path(e["output_folder"])/f"{e['scene_id']}_looped.mp4").exists())
    new_raw     = sum(1 for e in vid_entries if (Path(e["output_folder"])/f"{e['scene_id']}.mp4").exists()
                      and not (Path(e["output_folder"])/f"{e['scene_id']}_looped.mp4").exists())
    new_missing = len(vid_entries) - new_looped - new_raw
    imgs_done   = imgs_ready
