...
MANIFEST_FILE = BASE / "Production" / "clip_manifest.json"
...
def check_audio_layers() -> None:
    ...
    """
    beat_sheet = BASE / "Production" / "beat_sheet.json"
    score_dir  = BASE / "Audio_Stems" / "score"
    sfx_dir    = BASE / "Audio_Stems" / "sfx"
...
def print_status():
    ...
    looped  = sum(1 for e in vid_entries if (Path(e["output_folder"]) / f"{e['scene_id']}_looped.mp4").exists())
    raw     = sum(1 for e in vid_entries if (Path(e["output_folder"]) / f"{e['scene_id']}.mp4").exists()
                  and not (Path(e["output_folder"]) / f"{e['scene_id']}_looped.mp4").exists())
    pending = len(vid_entries) - looped - raw
    imgs    = sum(1 for e in img_entries if (Path(e["output_folder"]) / f"{e['scene_id']}.png").exists())
...
def run():
    ...
    for entry in img_entries:
        scene_id = entry["scene_id"]
        img_path = Path(entry["output_folder"]) / f"{scene_id}.png"
        if img_path.exists():
            continue
