...
VIDEO_DIR="002_Channels/001_Anomalous-Wild/Productions/0001_Bioluminescence_Weapon"
REMOTION_DIR="003_Remotion"
OUTPUT_FILE="002_Channels/001_Anomalous-Wild/Productions/0001_Bioluminescence_Weapon/Assembly/final_render.mp4"
...
for scene in scene_01 scene_03 scene_04 scene_05 scene_06 scene_07 scene_08 scene_09 scene_10a scene_10b scene_10c scene_11; do
  if [ ! -f "$VIDEO_DIR/Video_Clips/$scene.mp4" ]; then
    MISSING_VIDEO+=("$scene")
  fi
done
...
for scene in scene_01 scene_03 scene_04 scene_05 scene_06 scene_07 scene_08 scene_09 scene_10a scene_11 scene_12; do
  if [ ! -f "$VIDEO_DIR/Narration_Audio/$scene.mp3" ]; then
    MISSING_AUDIO+=("$scene")
  fi
done
...
if [ ${#MISSING_VIDEO[@]} -gt 0 ]; then
  ...
  for s in "${MISSING_VIDEO[@]}"; do echo "  - Video_Clips/$s.mp4"; done
  echo ""
  echo "Run: python3 004_Tools/run_video_gen_batch.py $VIDEO_DIR/Production/ai_prompts.json --priority all"
  echo ""
fi

if [ ${#MISSING_AUDIO[@]} -gt 0 ]; then
  echo "⚠ Missing audio files:"
  for s in "${MISSING_AUDIO[@]}"; do echo "  - Narration_Audio/$s.mp3"; done
  echo ""
  echo "Run: python3 004_Tools/run_tts_batch.py $VIDEO_DIR/Production/narration_tts.json"
  echo ""
fi
...
if [ ! -L "$REMOTION_DIR/public/bioluminescence_weapon" ]; then
  ...
  cd "$REMOTION_DIR/public" && ln -sf "../../002_Channels/001_Anomalous-Wild/Productions/0001_Bioluminescence_Weapon" bioluminescence_weapon && cd ../..
fi
