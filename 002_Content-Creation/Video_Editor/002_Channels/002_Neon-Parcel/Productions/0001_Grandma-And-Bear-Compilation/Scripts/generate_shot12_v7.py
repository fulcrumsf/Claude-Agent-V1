#!/usr/bin/env python3
"""Submit Shot 12 v7 from the approved eight-panel storyboard."""
...
PROMPT = ROOT / "Prompts/Shot-12-Seedance-2-Mini-v7.md"
STORYBOARD = ROOT / "Images/Shot-12-Storyboard-v3.png"
CONTRACT = CHANNEL_TOOLS / "shot_12_storyboard_spec_v3.json"
RAW = ROOT / "Working/Shot-12-Seedance-2-Mini-480p-v7.mp4"
LOG = ROOT / "Data/Generation_Log.json"
ANALYSIS = ROOT / "Working/Analysis/Shot-12-Storyboard-QA-v3"
...
def write_manifest(storyboard_url: str) -> Path:
    ...
    qa.write_text(json.dumps({"status": "pass", "review_mode": "tony_manual_approval", "findings": [], "checks": ["16:9_panels", "continuous_route", "trampoline_endpoint", "natural_scale", "single_camera"]}, indent=2) + "\n", encoding="utf-8")
    state = ANALYSIS / "attempt-state.json"
    state.write_text(json.dumps({"events": [{"event": "selected", "attempt": 1, "note": "Tony approved Shot 12 storyboard v3 with trampoline arrival and first bounce."}]}, indent=2) + "\n", encoding="utf-8")
    ...
    manifest.write_text(json.dumps({
        ...
        "reference_order": [{"upload_index": 1, "role": "approved eight-panel storyboard sheet"}],
        "manual_review_approved": True,
        "storyboard_sha256": sha256(STORYBOARD),
        "contract_sha256": sha256(CONTRACT),
        "qa_report_sha256": sha256(qa),
        "approval_note": "Tony approved Shot 12 storyboard v3 with Grandma arriving on the trampoline and beginning a small bounce.",
    }, indent=2) + "\n", encoding="utf-8")
    return manifest
...
def main() -> None:
    ...
    generate_seedance_mini(prompt, RAW, resolution="480p", aspect_ratio="16:9", duration=12, generate_audio=True, generation_log=LOG, shot_id="Shot-12", version="v7", prompt_file=PROMPT, retry_reason="tony_revision:approved storyboard v3 with trampoline bounce endpoint", reference_image_urls=[storyboard_url])
    data = json.loads(LOG.read_text(encoding="utf-8"))
    for item in data.get("assets", []):
        if item.get("shot_id") == "Shot-12" and item.get("version") == "v7":
