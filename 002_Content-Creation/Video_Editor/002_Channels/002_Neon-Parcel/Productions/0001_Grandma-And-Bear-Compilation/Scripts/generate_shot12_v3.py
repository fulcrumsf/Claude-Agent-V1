#!/usr/bin/env python3
"""Submit Shot 12 v3 using the approved climbing-step storyboard reference."""
...
PROMPT = ROOT / "Prompts/Shot-12-Seedance-2-Mini-v3.md"
STORYBOARD = ROOT / "Images/Shot-12-Storyboard-v2.png"
LOG = ROOT / "Data/Generation_Log.json"
RAW = ROOT / "Working/Shot-12-Seedance-2-Mini-480p-v3.mp4"
ANALYSIS = ROOT / "Working/Analysis/Shot-12-Storyboard-QA-v2"
CONTRACT = CHANNEL_TOOLS / "shot_12_storyboard_spec_v2.json"
...
def write_handoff(storyboard_url: str) -> Path:
    ...
    qa.write_text(json.dumps({
        ...
        "findings": [],
        "checks": ["16:9_panels", "subject_origin", "spatial_geometry", "chronology", "scale", "camera_continuity", "physics", "caption_layout"],
    }, indent=2) + "\n", encoding="utf-8")
    state = ANALYSIS / "attempt-state.json"
    state.write_text(json.dumps({"events": [{"event": "selected", "attempt": 2, "note": "Tony approved the climbing-step storyboard direction."}]}, indent=2) + "\n", encoding="utf-8")
    ...
    manifest.write_text(json.dumps({
        ...
        "approval_note": "Tony approved the Shot 12 storyboard direction showing Grandma climbing the steps at natural scale.",
    }, indent=2) + "\n", encoding="utf-8")
    return manifest
...
def main() -> None:
    ...
    shot = {
        ...
        "provider_verified_storyboard_sheet": False,
    ...
    generate_seedance_mini(
        ...
        version="v3",
        prompt_file=PROMPT,
        retry_reason="tony_revision:approved climbing-step storyboard v2",
        reference_image_urls=[storyboard_url],
    )
    data = json.loads(LOG.read_text(encoding="utf-8"))
    for item in data.get("assets", []):
        if item.get("shot_id") == "Shot-12" and item.get("version") == "v3":
