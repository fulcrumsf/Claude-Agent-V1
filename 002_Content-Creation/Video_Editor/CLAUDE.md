...
### Data Folder (single source of truth per production)
- Every production folder has one `Data/` subfolder — no separate `_review/`, `clip_manifest/`, or `Grading_Interview/` folders
- Everything production-tracking related lives here, as separate files:
  - **`Generation_Log.json`** — one entry per asset generated (image, video clip, VO take, SFX, music cue): model + version, platform, every prompt used (including rewrites and why), iteration count, issues found tagged by severity (🔴 Critical / 🟠 Major / 🟡 Minor), and a self-grade derived from severity × iteration count. Logging starts at the reference-image stage, not just final render — grade issues as soon as something looks off, before it goes to video.
  - **`Report_Card.md`** — human-readable rollup: Tony's grade, critique notes, workflow-change list. Generated by rolling up `Generation_Log.json` plus Tony's final grade/comments on the finished video, not hand-written from scratch.
  - Pipeline-specific data files (beat sheets, prop sheets, overlay configs, etc.) also live here per existing channel convention
- New productions must scaffold `Data/` automatically (see `scaffold_new_production.py` for Anomalous Wild)
