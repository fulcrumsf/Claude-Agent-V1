---
title: "Suno Text to Music — Generate Music"
type: spec
domain: video-production
tags: [spec, video-production, kie-ai, suno]
---
...
## Polling Endpoint — Get Music Task Details

> **The OpenAPI spec above does NOT include the polling endpoint.** Use this separately after submitting a generation task.
...
```json
{
  ...
  "msg": "success",
  "data": {
    "taskId": "e5f982be11e9b6b26c21116101dd8df5",
    "status": "SUCCESS",
    "response": {
      "taskId": "e5f982be11e9b6b26c21116101dd8df5",
      "sunoData": [
        {
          "id": "88b21a74-1416-48b2-9f43-52215efa8778",
          "audioUrl": "https://tempfile.aiquickdraw.com/r/<hash>.mp3",
          "sourceAudioUrl": "https://tempfile.aiquickdraw.com/r/<hash>.mp3",
          "streamAudioUrl": "https://musicfile.kie.ai/<base64id>",
          "sourceStreamAudioUrl": "https://cdn1.suno.ai/<id>.mp3",
          "imageUrl": "https://musicfile.kie.ai/<base64id>.jpeg",
          "sourceImageUrl": "https://cdn2.suno.ai/image_<id>.jpeg",
          "prompt": "[Instrumental]",
          "modelName": "chirp-auk",
          "title": "Sneaky Little Paws",
          "tags": "Bouncy upright piano with a playful, cartoony stride feel...",
          "createTime": 1774138399764,
          "duration": 125.88
        }
      ]
    },
...
| Value | Meaning |
|-------|---------|
| `SUCCESS` | Generation complete — `sunoData[].audioUrl` is ready to download |
| `PROCESSING` | Still generating — keep polling |
| `FAILED` | Generation failed — check `errorCode` / `errorMessage` |

### Notes
- The API generates **2 tracks** per request. Both are in `sunoData[]`.
- `audioUrl` is the primary download URL (MP3). Use this.
- `sourceStreamAudioUrl` is the direct Suno CDN URL — may expire faster.
- Files are retained for **14 days** from `createTime`.
- `callBackUrl` is required by the schema but can be a dummy URL (`https://api.example.com/callback`) if you are polling instead.
- Do NOT use `/api/v1/jobs/recordInfo` for music tasks — that endpoint is for video jobs only and will return 422.
- Do NOT use `GET /api/v1/generate?taskId=...` — returns "GET request not supported".
...
#!/usr/bin/env python3
"""
Generate a Suno instrumental track via kie.ai, poll for completion, download MP3.
Uses non-custom mode (simplest — only prompt required).
"""
import os, sys, time, json, requests

KIE_API_KEY = "YOUR_KIE_API_KEY"  # from .env: KIE_API_KEY
...
# ── Step 1: Submit ──────────────────────────────────────────────────────────
payload = {
    "prompt": "mischievous cat piano sounds quirky and funny",
    "customMode": False,       # simple mode — only prompt needed
    "instrumental": True,      # no vocals
    "model": "V4_5",           # V5 for best quality, V4_5 for speed
    "callBackUrl": "https://api.example.com/callback"  # required field, dummy ok if polling
}
r = requests.post(f"{BASE}/api/v1/generate", json=payload, headers=HEADERS)
d = r.json()
if d.get("code") != 200:
    print(f"Submit failed: {d}")
    sys.exit(1)
task_id = d["data"]["taskId"]
print(f"Task submitted: {task_id}")

# ── Step 2: Poll ─────────────────────────────────────────────────────────────
# Correct endpoint: GET /api/v1/generate/record-info?taskId=...
POLL_URL = f"{BASE}/api/v1/generate/record-info?taskId={task_id}"
audio_url = None
for i in range(60):  # poll up to 5 minutes (5s intervals)
    time.sleep(5)
    pr = requests.get(POLL_URL, headers=HEADERS)
    pd = pr.json()
    status = pd.get("data", {}).get("status")
    suno_data = pd.get("data", {}).get("response", {}).get("sunoData", [])
    if status == "SUCCESS" and suno_data and suno_data[0].get("audioUrl"):
        audio_url = suno_data[0]["audioUrl"]
        title = suno_data[0].get("title", "untitled")
        duration = suno_data[0].get("duration", 0)
        print(f"Ready: '{title}' ({duration:.1f}s) — {audio_url}")
        break
    if status == "FAILED":
        print(f"Generation failed: {pd.get('data', {}).get('errorMessage')}")
        sys.exit(1)
...
# ── Step 3: Download ──────────────────────────────────────────────────────────
ar = requests.get(audio_url, stream=True, timeout=60)
...
### Custom Mode Example (more control)

```python
payload = {
    "prompt": "",              # leave empty for instrumental custom mode
    "customMode": True,
    "instrumental": True,      # style + title required in custom mode
    "model": "V5",
    "style": "quirky upright piano, cartoony stride, playful, comedic",
    "title": "Sneaky Little Paws",
    "negativeTags": "vocals, lyrics, electronic, heavy metal",
    "callBackUrl": "https://api.example.com/callback"
}
```